  ...
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  @media (max-width: 768px) {{
    ...
    #layout {{
      ...
      height: auto;          /* Sin altura fija — crece con el contenido */
      gap: 16px;
    }}

    #col-izq {{
      width: 100%;
      height: 240px;         /* Altura explícita para que se vea */
      flex: none;            /* No heredar flex del padre */
    }}

    #col-der {{
      width: 100%;
      height: 380px;         /* Altura explícita para que position:absolute funcione */
      flex: none;
      position: relative;    /* Reconfirmar para que inset:0 funcione */
    }}

    /* Los hijos absolute NECESITAN que el padre tenga altura definida */
    #thumb,
    #video-wrapper {{
      position: absolute;
      top: 0; left: 0;
...
<script>
  ...
  function notifyHeight() {{
    const h = document.documentElement.scrollHeight + 40;
...
# ─── Altura del iframe: suficiente para móvil apilado (240+380+paddings) ───
...
<style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0;
    ...
    @media (max-width: 768px) {{
        ...
        /* Cada card ocupa todo el ancho, altura fija */
        .card {{
            flex: none;            /* No heredar flex */
            width: 100%;
            min-width: 0;
            max-width: 100%;
            aspect-ratio: unset;   /* Quitar aspect-ratio en móvil */
            height: 300px;         /* Altura fija explícita */
            min-height: unset;
        }}
    }}
</style>
...
<script>
    ...
    function notifyHeight() {{
        const h = document.documentElement.scrollHeight + 44;
        window.parent.postMessage({{ type: 'streamlit:setFrameHeight', height: h }}, '*');
    }}
...
# ─── Altura generosa para cubrir móvil (3 cards × 300px + paddings + título) ───
