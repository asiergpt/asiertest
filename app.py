...
/* TARJETAS DE PRODUCTOS (FUSIÓN DE A Y DIV PARA EVITAR BUGS DE STREAMLIT) */
...
.card-content {{ position: absolute; bottom: 20px; left: 20px; right: 20px; z-index: 5; color: white; display: flex; flex-direction: column; }}
...
.card:hover {{ transform: translateY(-10px); box-shadow: 0 20px 40px rgba(0,0,0,0.8), 0 0 35px 8px rgba(255, 204, 102, 0.2); }}
.card:hover .card-bg {{ opacity: 0.9; }}
.card:hover .card-btn {{ border-color: rgba(255, 204, 102, 0.5); background: rgba(255, 255, 255, 0.15); }}
...
.tech-circle {{ width: 70px; height: 70px; background: #111; border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 30px; border: 2px solid rgba(255,255,255,0.1); transition: 0.3s; box-sizing: border-box; }}
...
/* =========================================
   ADAPTACIÓN MÓVIL (BLINDADA CON !IMPORTANT)
   ========================================= */
@media (max-width: 768px) {{
    .top-layout, .cards-container, .footer-links {{ flex-direction: column; }}
    .f-divider {{ display: none; }}
    
    .cards-container {{ width: 100% !important; gap: 20px !important; display: flex; flex-direction: column; }}
    
    /* La clave absoluta para el ancho en móvil */
    .card {{ 
        width: 100% !important; 
    ...
    .black-block {{ padding: 40px 20px !important; width: 100% !important; box-sizing: border-box; }}
}}
</style>
...
<a href="https://asier-dorronsoro-busca-trabajo.streamlit.app/" target="_blank" class="card">
