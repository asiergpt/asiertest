...
/* Ocultamos elementos de Streamlit pero dejamos nuestra web intacta */
...
/* CABECERA CON SOMBRA DORADA SUTIL */
...
.col-izq:hover, .col-der:hover {{ 
    box-shadow: 0 15px 35px rgba(0,0,0,0.2), 0 0 25px 4px rgba(255, 204, 102, 0.15); 
...
/* TARJETAS CON SOMBRA DORADA FUERTE AL HOVER */
.cards-container {{ display: flex; gap: 24px; max-width: 1200px; margin: 0 auto; }}
.card-link {{ flex: 1; text-decoration: none; color: inherit; display: block; }}
.card {{ position: relative; border-radius: 16px; overflow: hidden; aspect-ratio: 3/4; background: #1a1a1a; transition: transform 0.4s ease, box-shadow 0.4s ease; height: 100%; }}
.card-bg {{ position: absolute; width: 100%; height: 100%; object-fit: cover; opacity: 0.7; transition: opacity 0.3s; }}
.card-gradient {{ position: absolute; bottom: 0; width: 100%; height: 70%; background: linear-gradient(to top, rgba(0,0,0,0.95), transparent); z-index: 2; }}
...
.card-subtitle {{ font-size: 13px; color: #ccc; margin-bottom: 12px; }}
...
.card-link:hover .card {{ 
    ...
    box-shadow: 0 20px 40px rgba(0,0,0,0.8), 0 0 35px 8px rgba(255, 204, 102, 0.2); 
}}
...
/* TECH STACK CON BARRA DE SCROLL ELEGANTE */
...
.tech-scroll {{ 
    ...
    scrollbar-color: rgba(255, 204, 102, 0.6) rgba(255, 255, 255, 0.05); 
}}

/* Estilos de Scrollbar para Chrome/Safari/Edge */
...
/* ADAPTACIÓN MÓVIL CORREGIDA */
@media (max-width: 768px) {{
    .top-layout, .cards-container, .footer-links {{ flex-direction: column; }}
    .f-divider {{ display: none; }}
    
    /* El ajuste clave para las tarjetas en móvil */
    .card-link {{ width: 100%; }}
    .card {{ 
        aspect-ratio: auto; /* Quitamos el ratio forzado que las aplastaba */
        height: 350px;      /* Altura fija y elegante */
        width: 100%;
    }}
    
    .col-der video {{ min-height: 250px; }}
    .black-block {{ padding: 40px 20px; }}
}}
</style>
...
<div class="card-subtitle">FAQ • Entrevistas IA</div>
...
<div class="card-subtitle">Ecosistema Empresarial Vasco</div>
<div class="card-btn">Explorar App</div>
</div>
</div>
</a>
...
<div class="card-subtitle">News & Network Vasco</div>
<div class="card-btn">Explorar App</div>
</div>
</div>
</a>
...
<div class="tech-item"><div class="tech-circle">🔌</div><span class="tech-name">API</span></div>
</div>
</div>
